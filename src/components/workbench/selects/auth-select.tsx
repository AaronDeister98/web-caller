...
export function AuthSelect(props: { onChange: (event: RequestProps) => void }) {
    ...
    const handleAuthTypeChange = (event: SelectChangeEvent) => {
        updateAuthState({ authType: event.target.value as 'basic' | 'bearer' | 'none', token: '', username: '', password: '' });
    }
    ...
    return (
        ...
        <FormControl fullWidth>
            ...
            {authState.authType !== 'none' && authState.authType !== 'basic' && <TextField
                label='Token'
                title='Token'
                variant="outlined"
                size='small'
            ...
            {authState.authType !== 'none' && authState.authType !== 'bearer' && <TextField
                label='Username'
                title='Username'
                variant="outlined"
                size='small'
